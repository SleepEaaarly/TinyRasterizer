...
template <typename T> struct vec<4,T> {
    vec() : x(T()), y(T()), z(T()), w(1.f) {}
    vec(T X, T Y, T Z) : x(X), y(Y), z(Z), w(1.f) {}
    vec(vec<3,T> &v) : x(v.x), y(v.y), z(v.z), w(1.f) {}
    vec(vec<3,T> &v, float w) : x(v.x), y(v.y), z(v.z), w(w) {}
    vec(T X, T Y, T Z, float w) : x(X), y(Y), z(Z), w(w) {}
          T& operator[](const size_t i)       { assert(i<4); return i<=0 ? x : (1==i ? y : (2==i ? z : w)); }
    const T& operator[](const size_t i) const { assert(i<4); return i<=0 ? x : (1==i ? y : (2==i ? z : w)); }
    float norm() { return std::sqrt(x*x+y*y+z*z); }
    vec<4,T> & clip() { 
        if (this->w < 1e-6f && this->w >= 0.f) {
            *this = (*this) / 1e-6f;
        } else if (this->w > -1e-6f && this->w < 0.f) {
            *this = (*this) / -1e-6f;
        } else {
            *this = (*this) / (this->w);
        }
    ...
    vec<3,T> value() { return vec<3,T>(x,y,z); }
